...
{
    public partial class KantorSetForm : Form
    {
        ...
        /// <param name="e"></param>
        private void KantorSetForm_Resize(object sender, EventArgs e)
        {
            if (bitmap == null)
            {
                widthOfLineTrack.Maximum = this.Width / 2 - 20;
                kantorSetPaintPanel.Width = this.Width / 2;
            }
            
            kantorSet = new KantorSet(widthOfLineTrack.Value, numOnRecursionTrack.Value, spaceBetweenLinesTrack.Value, numOnRecursionTrack.Value);
            bitmap = new Bitmap(kantorSetPaintPanel.Width, kantorSetPaintPanel.Height);
            graphics = Graphics.FromImage(bitmap);
            IEnumerable<Color> colors = GetGradients(startColor, endColor, numOnRecursionTrack.Value);
            kantorSet.DrawFractal(graphics, kantorSetPaintPanel, kantorSetPaintPanel.Width / 2 - widthOfLineTrack.Value / 2, kantorSetPaintPanel.Top + 50, widthOfLineTrack.Value, 1, colors);
            kantorSetPaintPanel.BackgroundImage = bitmap;
        }

        private void widthOfLineTrack_Move(object sender, EventArgs e)
        {
        ...
        private void spaceBetweenLinesTrack_ValueChanged(object sender, EventArgs e)
        {
            spaceBetweenLinesValue.Text = spaceBetweenLinesTrack.Value.ToString();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void drawKantorButton_Click(object sender, EventArgs e)
        {
            bitmap = new Bitmap(kantorSetPaintPanel.Width, kantorSetPaintPanel.Height);
            graphics = Graphics.FromImage(bitmap);
            kantorSet = new KantorSet(widthOfLineTrack.Value, numOnRecursionTrack.Value, spaceBetweenLinesTrack.Value, numOnRecursionTrack.Value);
            ...
            IEnumerable<Color> colors = GetGradients(startColor, endColor, numOnRecursionTrack.Value);
            kantorSet.DrawFractal(graphics, kantorSetPaintPanel, kantorSetPaintPanel.Width/2 - widthOfLineTrack.Value/2, 
                kantorSetPaintPanel.Top + 50, widthOfLineTrack.Value, 1, colors);
            kantorSetPaintPanel.BackgroundImage = bitmap;
        }
